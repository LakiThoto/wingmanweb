...
// Since we're in node environment, we simulate the body attribute logic
// by testing the CSS class rules as JavaScript predicates.

type Tier = 'beginner' | 'experienced' | 'pro';

function isVisible(tier: Tier, classes: string[]): boolean {
  if (classes.includes('beginner-only') && tier !== 'beginner') return false;
  if (classes.includes('experienced-only') && tier !== 'experienced') return false;
  if (classes.includes('pro-only') && tier !== 'pro') return false;
  if (classes.includes('pro-hide') && tier === 'pro') return false;
  return true;
}
...
describe('tier strings — button labels switch correctly', () => {
  ...
  it('both labels cover all tiers (no tier leaves a button label-less)', () => {
    const tiers: Tier[] = ['beginner', 'experienced', 'pro'];
    tiers.forEach(tier => {
