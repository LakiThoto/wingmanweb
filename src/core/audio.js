...
function resolveTierString(key) {
...
function setAiSpeaking(durationMs) {
...
export function flashAiConfirmed() {
    const btn = document.querySelector('.btn-primary');
    if (!btn)
        return;
...
function playAudioFile(filename, fallbackText) {
    const audio = new Audio(`/audio/${filename}`);
    const useFallback = () => {
        if (fallbackText)
            speakTtsText(fallbackText);
    };
...
function substituteVars(text, vars) {
    if (!vars)
        return text;
    return Object.entries(vars).reduce((s, [k, v]) => s.replace(new RegExp(`\\{${k}\\}`, 'g'), v), text);
}
function speakResolvedText(variant) {
...
export function speakByTier(key) {
    ...
    if (mode === 'glasses') {
        ...
        if (filename)
            playAudioFile(filename, variant);
        else
            speakTtsText(variant);
        return;
    }
