...
export function formatDutchPlate(raw: string): string | null {
...
export function parsePlateFromSpeech(transcript: string): string | null {
  ...
  if (withGaps) return formatDutchPlate(`${withGaps[1]}${withGaps[2]}${withGaps[3]}`);

  const compact = text.replace(/[^A-Z0-9]/g, '');
  const compactMatch = compact.match(/([A-Z]{2}\d{2,3}[A-Z]{1,2})/);
  if (compactMatch) return formatDutchPlate(compactMatch[1]);

  return formatDutchPlate(compact);
}

export function isValidDutchPlate(raw: string): boolean {
