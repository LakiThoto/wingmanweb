...
export type CustomVoiceTier = 'beginner' | 'experienced' | 'pro';

export interface CustomSettings {
  audio: number;
  support: number;
  voiceTier: CustomVoiceTier;
}
...
let draft: CustomSettings = { ...DEFAULTS };

function clampPct(n: number): number {
  return Math.max(0, Math.min(100, Math.round(n)));
}

function load(): CustomSettings {
  try {
    ...
    return {
      ...
      voiceTier:
        parsed.voiceTier === 'experienced' || parsed.voiceTier === 'pro'
          ? parsed.voiceTier
          : 'beginner',
    };
  } catch {
...
function persist(settings: CustomSettings): void {
...
export function resetCustomDraft(): void {
  draft = { ...load() };
}

export function getCustomDraft(): Readonly<CustomSettings> {
  return draft;
}

export function setCustomDraftAudio(pct: number): void {
  draft = { ...draft, audio: clampPct(pct) };
  syncCustomDataAttributes();
  emit('custom_settings_change', { ...draft });
}

export function setCustomDraftSupport(pct: number): void {
  draft = { ...draft, support: clampPct(pct) };
  syncCustomDataAttributes();
  emit('custom_settings_change', { ...draft });
}

export function adjustCustomDraftSupport(delta: number): number {
  const next = clampPct(draft.support + delta);
  setCustomDraftSupport(next);
  return next;
}

export function setCustomDraftVoiceTier(tier: CustomVoiceTier): void {
  draft = { ...draft, voiceTier: tier };
  emit('custom_settings_change', { ...draft });
}

/** Persist draft, activate custom tier, apply support-derived UI density. */
export function saveCustomSettings(): CustomSettings {
  draft = {
...
export type DensityTier = 'beginner' | 'experienced' | 'pro';

/** Map support % to hint density while in custom mode. */
export function supportToDensityTier(support: number): DensityTier {
  if (support <= 33) return 'pro';
  if (support <= 66) return 'experienced';
  return 'beginner';
}

export function syncCustomDataAttributes(): void {
  const s = draft;
