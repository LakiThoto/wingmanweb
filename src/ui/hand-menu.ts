...
import {
  resetCustomDraft,
  setCustomDraftAudio,
  setCustomDraftSupport,
  setCustomDraftVoiceTier,
  adjustCustomDraftSupport,
  saveCustomSettings,
  getCustomDraft,
  type CustomVoiceTier,
} from '@/core/custom-settings';
...
let gesturePulseTimer: ReturnType<typeof setTimeout> | null = null;

function getScreenStage(): HTMLElement | null {
  return document.querySelector('#app .screen-stage');
}

function attachHandMenuToStage(): void {
  if (!overlayEl) return;
  const stage = getScreenStage();
  if (!stage) return;
  if (overlayEl.parentElement !== stage) stage.appendChild(overlayEl);
}

function setStageMenuOpen(open: boolean): void {
...
export function onHandMenuCustomSupportChange(delta: number): void {
...
function circleTile(
  ...
  inner: string,
  active: boolean,
  opts: { tier?: Tier | 'custom'; lang?: AppLang; voiceTier?: CustomVoiceTier } = {},
): string {
...
function setSliderUi(
  ...
  const valueEl = overlayEl?.querySelector<HTMLElement>(`#${valueId}`);
  const fillEl = overlayEl?.querySelector<HTMLElement>(`#${fillId}`);
  if (valueEl) valueEl.textContent = `${pct}%`;
  if (fillEl) fillEl.style.width = `${pct}%`;
}

function syncCustomPanelUi(): void {
  if (!overlayEl) return;
  const draft = getCustomDraft();
  ...
  overlayEl.querySelectorAll<HTMLButtonElement>('[data-voice-tier]').forEach(btn => {
...
function showMainMenuView(): void {
  if (!overlayEl) return;
  isCustomView = false;
...
function showCustomMenuView(): void {
  if (!overlayEl) return;
  isCustomView = true;
  resetCustomDraft();
  ...
  requestAnimationFrame(() => {
    overlayEl?.querySelector<HTMLElement>('#btn-custom-save')?.focus();
...
function attachHandlers(): void {
  ...
  overlayEl.querySelector('#menu-custom-support')?.addEventListener('click', (e) => {
    const track = (e.currentTarget as HTMLElement).querySelector('.menu-slider-track');
    if (!track) return;
...
export function openHandMenu(): void {
  if (!overlayEl || !canOpenHandMenu()) return;
  ...
  syncMenuSelection();
  requestAnimationFrame(() => {
    const first = overlayEl?.querySelector<HTMLElement>('.menu-panel--main .menu-circle-tile');
    first?.focus();
...
export function toggleHandMenu(): void {
  ...
  else if (canOpenHandMenu()) openHandMenu();
}
