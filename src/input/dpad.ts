...
function getFocusables(root: Element = document.body): HTMLElement[] {
  return Array.from(root.querySelectorAll<HTMLElement>('.focusable[tabindex="0"]')).filter(
    isFocusable,
  );
}
...
function getBottomBarFocusables(): HTMLElement[] {
  ...
  document.querySelectorAll<HTMLElement>(BOTTOM_CTA_LAYER_SEL).forEach(layer => {
    ...
    getFocusables(layer).forEach(el => {
      if (!ring.includes(el)) ring.push(el);
    });
  });
...
function isBottomHorizontalGroup(group: HTMLElement): boolean {
  return !!group.closest(BOTTOM_CTA_LAYER_SEL);
}

function tryMoveBottomBar(direction: 'prev' | 'next'): boolean {
...
function moveFocus(direction: 'prev' | 'next', axis: 'vertical' | 'horizontal'): void {
  ...
  if (axis === 'horizontal') {
    if (tryMoveBottomBar(direction)) return;
