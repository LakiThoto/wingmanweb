...
import type { AppState, Delivery } from '@/types';
...
function buildLockerFindScreen(
...
function buildLockerPlaceScreen(delivery: Delivery | undefined): string {
  const row = delivery?.rowInVan ?? 'B';
  const pos = delivery?.positionInRow ?? 1;
...
function buildLockerPrintScreen(delivery: Delivery | undefined): string {
...
export function mount(container: HTMLElement): () => void {
  ...
  let printConfirmTimer: ReturnType<typeof setTimeout> | null = null;

  function clearPrintConfirmTimer(): void {
  ...
  function finishPrintConfirm(): void {
