...
let packageFoundHandler: (() => void) | null = null;

/** Voice / external trigger while on zoek. */
export function requestZoekPackageFound(): boolean {
  if (getState().screen !== 'zoek' || !packageFoundHandler) return false;
  packageFoundHandler();
  return true;
}

function buildZoekSearchMarkup(
...
export function mount(container: HTMLElement): () => void {
  ...
  const goToWalk = () => {
    clearAdvance();
    if (getState().screen === 'zoek') transition('pkg_confirmed');
  };
